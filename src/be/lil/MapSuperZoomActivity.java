...
public class MapSuperZoomActivity extends Activity {
  ...
  private static String TAG = "LIL - MapSuperZoomActivity";
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    for (Spot spot : allSpots) {
      CoordinatesTranslator.addMapCoordinates(spot, CoordinatesTranslator.SUPERZOOM);
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    inflater.inflate(R.menu.superzoommapmenu, menu);
    return true;
  }
  ...
  private List<Coordinates> getRoutes(long spot_id){
    ...
    try{
      ...
      while (cursor.moveToNext()) {
        ...
        CoordinatesTranslator.addMapCoordinates(includesMapData, CoordinatesTranslator.SUPERZOOM);
  ...
  private class SampleView extends View {
    ...
    protected void moveToCenter() {
      if(startCenterX<0 && startCenterY<0){
        ...
        startX = CoordinatesTranslator.getMapX(startCenterX, CoordinatesTranslator.SUPERZOOM) - displayWidth/2;
        startY = CoordinatesTranslator.getMapY(startCenterY, CoordinatesTranslator.SUPERZOOM) - displayHeight/2;
    ...
    private Spot isOnSpot(float mapX, float mapY){
      float realX = CoordinatesTranslator.getRealX(mapX+scrollRectX, CoordinatesTranslator.SUPERZOOM);
      float realY = CoordinatesTranslator.getRealY(mapY+scrollRectY, CoordinatesTranslator.SUPERZOOM);
      for (Spot currentSpot : allSpots) {
        float absX = 1000*Math.abs(currentSpot.getX()-realX);
        float absY = 1000*Math.abs(currentSpot.getY()-realY);
        if(absX < 0.1 && absY < 0.1) return currentSpot;
      }
      return null;
    }

    /*private void processSpotClick(float mapX, float mapY){
      float realX = CoordinatesTranslator.getRealX(mapX+scrollRectX, CoordinatesTranslator.SUPERZOOM);
      float realY = CoordinatesTranslator.getRealY(mapY+scrollRectY, CoordinatesTranslator.SUPERZOOM);
      for (Spot currentSpot : allSpots) {
        float absX = 1000*Math.abs(currentSpot.getX()-realX);
        ...
        if(absX < 0.1 && absY < 0.1)
          showSpot(currentSpot.getSpotorder());
      }
    }*/
  }
}