...
public class MapDetailActivity extends Activity {
  ...
  private static String TAG = "LIL - MapDetailActivity";
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    for (Spot spot : allSpots) {
      CoordinatesTranslator.addMapCoordinates(spot, CoordinatesTranslator.DETAIL);
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    inflater.inflate(R.menu.detailmapmenu, menu);
    return true;
  }
  ...
  private void createSpotsList(Cursor cursor) {
    ...
    while (cursor.moveToNext()) {
      allSpots.add(new Spot(cursor.getLong(0), cursor.getString(1), cursor.getFloat(2), cursor.getFloat(3), cursor.getInt(4)));
    }
  }
  ...
  private List<Coordinates> getRoutes(long spot_id){
    ...
    try{
      ...
      while (cursor.moveToNext()) {
        ...
        CoordinatesTranslator.addMapCoordinates(includesMapData, CoordinatesTranslator.DETAIL);
  ...
  private class SampleView extends View {
    ...
    public SampleView(Context context, float centerX, float centerY, long targetSpotId, float meX, float meY) {
      ...
      bmLargeImage = BitmapFactory.decodeResource(getResources(),
        R.drawable.lost_map_no_icons_zoom, options);
      ...
      if(meX!=0 && meY!=0){
        mapMeX = CoordinatesTranslator.getMapX(meX, CoordinatesTranslator.DETAIL);
        mapMeY = CoordinatesTranslator.getMapY(meY, CoordinatesTranslator.DETAIL);
      }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
      ...
      switch (event.getAction()) {
        case MotionEvent.ACTION_DOWN:
          ...
          if(isOnZoomIn(event.getRawX(), event.getRawY())){
            zoomIn(
              CoordinatesTranslator.getRealX(scrollRectX+displayWidth/2, CoordinatesTranslator.DETAIL),
              CoordinatesTranslator.getRealY(scrollRectY+displayHeight/2, CoordinatesTranslator.DETAIL));
            break;
          }
          else if(isOnZoomOut(event.getRawX(), event.getRawY())){
            zoomOut(
              CoordinatesTranslator.getRealX(scrollRectX+displayWidth/2, CoordinatesTranslator.DETAIL),
              CoordinatesTranslator.getRealY(scrollRectY+displayHeight/2, CoordinatesTranslator.DETAIL));
            break;
          }
          Spot clickedSpot = isOnSpot(event.getRawX(), event.getRawY());
          if(clickedSpot != null)
            showSpot(clickedSpot.getSpotorder());
    ...
    protected void onDraw(Canvas canvas) {
      ...
      for (Spot currentSpot : allSpots) {
        ...
        canvas.drawBitmap(bm, null, new Rect(
          (int)(currentSpot.getMapX()-newScrollRectX-10),
          (int)(currentSpot.getMapY()-newScrollRectY-40),
          (int)(currentSpot.getMapX()-newScrollRectX+10),
      ...
      canvas.drawRect(new Rect(displayWidth-141, displayHeight-74, displayWidth-7, displayHeight-7), paint);
      paint.setColor(0xff000000);
      Bitmap zoom_out = BitmapFactory.decodeResource(getResources(), R.drawable.zoom_out_60);
      canvas.drawBitmap(zoom_out, displayWidth-134, displayHeight-67, paint);
      Bitmap zoom_in = BitmapFactory.decodeResource(getResources(), R.drawable.zoom_in_60);
      canvas.drawBitmap(zoom_in, displayWidth-67, displayHeight-67, paint);*/
    ...
    protected void moveToCenter() {
      if(startCenterX<0 && startCenterY<0){
        ...
        startX = CoordinatesTranslator.getMapX(startCenterX, CoordinatesTranslator.DETAIL) - displayWidth/2;
        startY = CoordinatesTranslator.getMapY(startCenterY, CoordinatesTranslator.DETAIL) - displayHeight/2;
    ...
    private boolean isOnZoomOut(float x, float y){
      if(74<(displayWidth-x) && (displayWidth-x)<134 && 7<(displayHeight-y) && (displayHeight-y)<67) return true;
      return false;
    }

    private boolean isOnZoomIn(float x, float y){
      if(7<(displayWidth-x) && (displayWidth-x)<67 && 7<(displayHeight-y) && (displayHeight-y)<67) return true;
      return false;
    }

    private Spot isOnSpot(float mapX, float mapY){
      float realX = CoordinatesTranslator.getRealX(mapX+scrollRectX, CoordinatesTranslator.DETAIL);
      float realY = CoordinatesTranslator.getRealY(mapY+scrollRectY, CoordinatesTranslator.DETAIL);
      for (Spot currentSpot : allSpots) {
        float absX = 1000*Math.abs(currentSpot.getX()-realX);
        float absY = 1000*Math.abs(currentSpot.getY()-realY);
        if(absX < 0.25 && absY < 0.25) return currentSpot;
      }
      return null;
    }
  }
